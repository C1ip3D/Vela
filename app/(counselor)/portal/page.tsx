...
const riskLabel = (risk: string) =>
  risk === "high" ? "High Risk" : risk === "medium" ? "Monitor" : "On Track";

export default function CounselorPortalPage() {
  // Data is populated by the Canvas sync pipeline.
  // No students or alerts until real data is connected.
  const students: any[] = [];
  const alerts: any[] = [];
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <TopBar title="Counselor Portal" studentName="Counselor" />
      ...
      <div className="flex-1 p-6 space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {[
            ...
            { label: "Pending Alerts", value: pendingAlerts, warn: pendingAlerts > 0 },
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-3">
            ...
            {students.length === 0 ? (
              <div className="rounded-xl border border-[#1F2D4A] bg-[#141C33] px-6 py-12 flex flex-col items-center gap-3 text-center">
                <div className="h-10 w-10 rounded-full bg-[#1A2340] flex items-center justify-center">
                  <FileText size={18} className="text-[#475569]" />
                </div>
                <p className="text-sm text-[#94A3B8]">No students to display</p>
                <p className="text-xs text-[#475569] max-w-xs">
                  Students will appear here once they connect Canvas and sync their data.
                </p>
              </div>
            ) : (
          ...
          <div className="space-y-3">
            ...
            {alerts.length === 0 ? (
              <div className="rounded-xl border border-[#1F2D4A] bg-[#141C33] px-4 py-10 flex flex-col items-center gap-3 text-center">
                <div className="h-8 w-8 rounded-full bg-[#1A2340] flex items-center justify-center">
                  <AlertTriangle size={15} className="text-[#475569]" />
                </div>
                <p className="text-sm text-[#94A3B8]">No alerts</p>
                <p className="text-xs text-[#475569]">Alerts appear when students have grade issues or missing assignments.</p>
              </div>
            ) : (
              alerts.map((alert) => (
                <div key={alert.id} className={`rounded-xl border bg-[#141C33] p-3 ${alert.severity === "CRITICAL" ? "border-l-2 border-rose-500 border-[#1F2D4A]" : "border-l-2 border-amber-400 border-[#1F2D4A]"}`}>
                  <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-medium text-[#E2E8F0]">{alert.studentName}</p>
