...
// GET — no profile, no recommendations
...
export async function POST(req: Request) {
  try {
    ...
    // 1. Catalog-based recommendations — require a real student profile
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let catalogRecs: any[] = [];
    if (studentProfile) {
      catalogRecs = getRecommendations(studentProfile);
    } else {
      // No profile provided — nothing to recommend
      return NextResponse.json({ recommendations: [], aiEnhanced: false });
    }
